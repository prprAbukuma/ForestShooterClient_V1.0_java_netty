...
public class PlayerManager : BaseManager {
    ...
    //游戏中的角色数据
    public Dictionary<RoleColor, RoleData> roleDataDict = new Dictionary<RoleColor, RoleData>();
    //生成位置
    private Transform _redRoleSpawnPosition;
    private Transform _blueRoleSpawnPosition;
    //当前控制的本地角色
   // private RoleType _currentControllRoleType;//弃用
    private RoleColor _currentControllRoleColor;
    ...
    {
        //目前只有两个游戏角色
        RoleData hunter_blue = new RoleData(RoleColor.Blue, "Prefabs/Hunter_BLUE", "Prefabs/Arrow_Blue");
        RoleData hunter_red = new RoleData(RoleColor.Red, "Prefabs/Hunter_RED", "Prefabs/Arrow_Red");
        //加入字典
        roleDataDict.Add(hunter_blue.RoleColor, hunter_blue);
        roleDataDict.Add(hunter_red.RoleColor, hunter_red);
    }
    /// <summary>
    ...
    public void SpawnAllRoles()
    {
        ...
        {
            switch (roleData.RoleColor)
            {
                case RoleColor.Blue:
                    hunter_blue= GameObject.Instantiate(roleDataDict[RoleColor.Blue].RolePrefab, _blueRoleSpawnPosition.position, Quaternion.identity);
                    //并设置他们的标签
                    hunter_blue.tag = "Blue";
                    if (_currentControllRoleColor == RoleColor.Blue)
                    {
                        _currentControllRoleGo = hunter_blue;
                    }
                    break;
                case RoleColor.Red:
                    hunter_red = GameObject.Instantiate(roleDataDict[RoleColor.Red].RolePrefab, _redRoleSpawnPosition.position, Quaternion.identity);
                    //并设置他们的标签
                    hunter_red.tag = "Red";
                    if (_currentControllRoleColor == RoleColor.Red)
                    {
